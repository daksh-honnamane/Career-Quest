...
class ChefScene:
    def __init__(self, game):
        ...
        self.prep_mistakes = 0
        self.prep_hint_used = False
        self.station_mistakes = 0
        self.plating_mistakes = 0
        self.plating_hint_used = False
    ...
    def handle_event(self, event):
        ...
        if self.active_stage not in {"overview", "results"} and self._hub_button_rect().collidepoint(event.pos):
            self.return_to_hub()
            return True
        ...
        if self.active_stage == "overview":
            ...
            if self._hint_button_rect().collidepoint(event.pos) or self._action_button_rect().collidepoint(event.pos):
            ...
            if self.service_finished and self._continue_button_rect().collidepoint(event.pos):
    ...
    def render(self, screen):
        ...
        if self.active_stage not in {"overview", "results"}:
            self.draw_button(screen, self._hub_button_rect(), "Hub")
        ...
        for index, rect in enumerate(self._prep_card_rects()):
            ...
            if card == self.prep_correct_order[next_index]:
                ...
                if len(self.prep_completed) == len(self.prep_correct_order):
                    self.skill_scores["organization"] = self._score_performance(1.0, self.prep_mistakes, self.prep_hint_used)
        ...
        for option, rect in self._station_option_rects().items():
            ...
            if option == round_data["correct"]:
                ...
                if self.station_round_index >= len(self.station_rounds):
                    self.skill_scores["timing"] = self._score_performance(1.0, self.station_mistakes)
                    self.advance_stage()
            else:
                self.station_mistakes += 1
                self.station_feedback = f"Not quite. {option} would throw off the dish timing."
            return

    def handle_plating_click(self, pos):
        if self._hint_button_rect().collidepoint(pos):
            self.plating_hint_used = True
        ...
        if self._action_button_rect().collidepoint(pos):
            if self.plating_slots == self.plating_correct_order:
                self.skill_scores["precision"] = self._score_performance(1.0, self.plating_mistakes, self.plating_hint_used)
                self.plating_feedback = "Plate approved. Clean structure helps food look intentional."
                self.advance_stage()
            else:
                self.plating_mistakes += 1
    ...
    def complete_service_sprint(self):
        ...
        success_ratio = self.service_hits / len(self.service_rounds)
        self.skill_scores["composure"] = self._score_performance(success_ratio, self.service_misses)
    ...
    def render_results(self, screen):
        ...
        for index, (label, value) in enumerate(ratings):
            ...
            self.draw_text(screen, f"{value}/5", small_font, TEXT_COLOR, card_rect.x + 18, card_rect.bottom - small_font.get_height() - 14)
    ...
    def _score_performance(self, success_ratio, mistakes=0, hint_used=False):
        raw_score = (success_ratio * 5) - (mistakes * 0.6) - (1.0 if hint_used else 0.0)
        return max(0, min(5, int(raw_score)))
    ...
    def _draw_chef_avatar(self, screen, rect):
        ...
        top_y = rect.y + max(16, int(20 * scale))
        skin_color = (219, 186, 154)
        hair_color = (92, 76, 62)
        hat_color = (249, 247, 242)
        band_color = (230, 224, 214)
        coat_color = (247, 247, 244)
        apron_color = (214, 198, 171)

        hat_top = [
            (center_x - max(38, int(44 * scale)), top_y + max(48, int(52 * scale))),
            (center_x - max(28, int(34 * scale)), top_y + max(22, int(24 * scale))),
            (center_x - max(6, int(8 * scale)), top_y + max(16, int(18 * scale))),
            (center_x + max(18, int(20 * scale)), top_y + max(20, int(22 * scale))),
            (center_x + max(36, int(40 * scale)), top_y + max(44, int(48 * scale))),
            (center_x + max(22, int(26 * scale)), top_y + max(60, int(66 * scale))),
            (center_x - max(18, int(22 * scale)), top_y + max(62, int(68 * scale))),
        ]
        hat_band = [
            (center_x - max(42, int(48 * scale)), top_y + max(60, int(64 * scale))),
            (center_x + max(42, int(48 * scale)), top_y + max(60, int(64 * scale))),
            (center_x + max(34, int(38 * scale)), top_y + max(76, int(82 * scale))),
            (center_x - max(34, int(38 * scale)), top_y + max(76, int(82 * scale))),
        ]
        head = [
            (center_x - max(26, int(30 * scale)), top_y + max(82, int(88 * scale))),
            (center_x - max(16, int(20 * scale)), top_y + max(62, int(66 * scale))),
            (center_x + max(16, int(20 * scale)), top_y + max(62, int(66 * scale))),
            (center_x + max(26, int(30 * scale)), top_y + max(82, int(88 * scale))),
            (center_x + max(18, int(22 * scale)), top_y + max(120, int(126 * scale))),
            (center_x - max(18, int(22 * scale)), top_y + max(120, int(126 * scale))),
        ]
        hair = [
            (center_x - max(26, int(30 * scale)), top_y + max(82, int(86 * scale))),
            (center_x - max(12, int(16 * scale)), top_y + max(58, int(60 * scale))),
            (center_x + max(22, int(24 * scale)), top_y + max(62, int(64 * scale))),
            (center_x + max(28, int(32 * scale)), top_y + max(86, int(90 * scale))),
            (center_x + max(8, int(10 * scale)), top_y + max(76, int(80 * scale))),
            (center_x - max(16, int(18 * scale)), top_y + max(74, int(78 * scale))),
        ]
        coat_top = top_y + max(136, int(144 * scale))
        coat_bottom = rect.bottom - max(28, int(34 * scale))
        coat = [
            (center_x - max(68, int(76 * scale)), coat_bottom),
            (center_x - max(48, int(54 * scale)), coat_top + max(14, int(16 * scale))),
            (center_x - max(18, int(22 * scale)), coat_top),
            (center_x + max(18, int(22 * scale)), coat_top),
            (center_x + max(48, int(54 * scale)), coat_top + max(14, int(16 * scale))),
            (center_x + max(68, int(76 * scale)), coat_bottom),
        ]
        apron = [
            (center_x - max(22, int(24 * scale)), coat_top + max(4, int(6 * scale))),
            (center_x + max(22, int(24 * scale)), coat_top + max(4, int(6 * scale))),
            (center_x + max(14, int(16 * scale)), coat_top + max(52, int(58 * scale))),
            (center_x - max(14, int(16 * scale)), coat_top + max(52, int(58 * scale))),
        ]
        neck = [
            (center_x - max(12, int(14 * scale)), head[5][1] - max(2, int(3 * scale))),
            (center_x + max(12, int(14 * scale)), head[4][1] - max(2, int(3 * scale))),
            (center_x + max(18, int(20 * scale)), coat_top + max(6, int(8 * scale))),
            (center_x - max(18, int(20 * scale)), coat_top + max(6, int(8 * scale))),
        ]

        pygame.draw.polygon(screen, hat_color, hat_top)
        pygame.draw.polygon(screen, band_color, hat_band)
        ...
        pygame.draw.polygon(screen, coat_color, coat)
        pygame.draw.polygon(screen, apron_color, apron)
        pygame.draw.line(screen, TEXT_COLOR, (center_x - max(10, int(12 * scale)), top_y + max(94, int(100 * scale))), (center_x - max(2, int(4 * scale)), top_y + max(94, int(100 * scale))), 2)
        pygame.draw.line(screen, TEXT_COLOR, (center_x + max(2, int(4 * scale)), top_y + max(94, int(100 * scale))), (center_x + max(10, int(12 * scale)), top_y + max(94, int(100 * scale))), 2)
        pygame.draw.line(screen, TEXT_COLOR, (center_x - max(8, int(10 * scale)), top_y + max(112, int(118 * scale))), (center_x + max(8, int(10 * scale)), top_y + max(112, int(118 * scale))), 2)
    ...
    def _hub_button_rect(self):
        scale = self._ui_scale()
        width = self.game.screen.get_width()
        button_width = max(96, int(124 * scale))
        button_height = max(40, int(48 * scale))
        margin_top = max(18, int(20 * scale))
        margin_right = max(24, int(28 * scale))
        return pygame.Rect(width - button_width - margin_right, margin_top, button_width, button_height)
    ...
    def _results_layout_rects(self):
        ...
        content_top = panel_rect.y + max(150, int(176 * scale))
        gap = max(16, int(20 * scale))
