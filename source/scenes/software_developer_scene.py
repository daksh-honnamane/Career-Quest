...
class SoftwareDeveloperScene:
    def __init__(self, game):
        ...
        self.bug_mistakes = 0
        self.bug_hint_used = False
        self.logic_mistakes = 0
        self.logic_hint_used = False
        self.optimization_mistakes = 0
        self.pressure_misses = 0
        ...
        self.pressure_rounds = [
            self._build_pressure_round(['Line 3: pritn("Hello")', 'Line 7: score += 1', 'Line 10: return score'], 0),
            self._build_pressure_round(['Line 2: if score = 10', 'Line 4: print(score)', 'Line 6: total += bonus'], 0),
            self._build_pressure_round(['Line 1: usernme = input()', 'Line 5: password = getpass()', 'Line 8: return True'], 0),
            self._build_pressure_round(['Line 9: for i in range(3)', 'Line 11: print(i)', 'Line 12: done = True'], 0),
            self._build_pressure_round(['Line 3: items.apend(task)', 'Line 6: len(items)', 'Line 7: save(items)'], 0),
            self._build_pressure_round(['Line 5: while ready == True:', 'Line 9: result = run()', 'Line 10: return result'], 0),
        ]
    ...
    def handle_event(self, event):
        ...
        if self.active_stage == "overview":
            ...
            if option_a_rect.collidepoint(event.pos) or option_b_rect.collidepoint(event.pos):
                self.handle_optimization_click(event.pos)
                return True
            self.handle_optimization_click(event.pos)
        elif self.active_stage == "pressure_debug":
            if self.pressure_finished and self._continue_button_rect().collidepoint(event.pos):
    ...
    def handle_bug_hunt_click(self, pos):
        ...
        if self._hint_button_rect().collidepoint(pos):
            ...
            if self.bug_selected_line == self.bug_error_line:
                self.skill_scores["debugging"] = self._score_performance(1.0, self.bug_mistakes, self.bug_hint_used)
    ...
    def handle_logic_click(self, pos):
        ...
        if self._action_button_rect().collidepoint(pos):
            if self.logic_slots == self.logic_correct_order:
                self.skill_scores["logic"] = self._score_performance(1.0, self.logic_mistakes, self.logic_hint_used)
                self.logic_feedback = "Correct. Good systems design follows a safe sequence."
                self.advance_stage()
            else:
                self.logic_mistakes += 1
    ...
    def handle_optimization_click(self, pos):
        ...
        if option_a_rect.collidepoint(pos):
            ...
            self.skill_scores["efficiency"] = self._score_performance(1.0, self.optimization_mistakes)
    ...
    def complete_pressure_debug(self):
        ...
        success_ratio = self.pressure_hits / len(self.pressure_rounds)
        self.skill_scores["speed"] = self._score_performance(success_ratio, self.pressure_misses)
    ...
    def render_dialogue_stage(self, screen):
        ...
        self._draw_software_developer_avatar(screen, avatar_rect)
    ...
    def _build_pressure_round(self, lines, bug_index):
        shuffled_lines = lines[:]
        correct_line = shuffled_lines[bug_index]
        random.shuffle(shuffled_lines)
        return {"lines": shuffled_lines, "bug_index": shuffled_lines.index(correct_line)}
    ...
    def _draw_software_developer_avatar(self, screen, rect):
        scale = self._ui_scale()
        center_x = rect.centerx
        top_y = rect.y + max(18, int(22 * scale))
        skin_color = (219, 186, 154)
        hair_color = (59, 68, 86)
        jacket_color = (73, 110, 156)
        shirt_color = (234, 241, 248)
        laptop_color = (72, 86, 110)
        screen_color = (206, 228, 246)
        ...
        torso_bottom = rect.bottom - max(32, int(38 * scale))
        torso = [
        ...
        laptop = [
            (center_x - max(54, int(62 * scale)), rect.bottom - max(70, int(78 * scale))),
            (center_x + max(54, int(62 * scale)), rect.bottom - max(70, int(78 * scale))),
            (center_x + max(68, int(78 * scale)), rect.bottom - max(22, int(26 * scale))),
            (center_x - max(68, int(78 * scale)), rect.bottom - max(22, int(26 * scale))),
        ]
        laptop_screen = [
            (center_x - max(40, int(46 * scale)), rect.bottom - max(64, int(72 * scale))),
            (center_x + max(40, int(46 * scale)), rect.bottom - max(64, int(72 * scale))),
            (center_x + max(50, int(58 * scale)), rect.bottom - max(34, int(38 * scale))),
            (center_x - max(50, int(58 * scale)), rect.bottom - max(34, int(38 * scale))),
        ]
        ...
        pygame.draw.polygon(screen, laptop_color, laptop)
        pygame.draw.polygon(screen, screen_color, laptop_screen)
