...
class DoctorScene:
    def __init__(self, game):
        ...
        self.triage_mistakes = 0
        self.triage_hint_used = False
        self.diagnosis_mistakes = 0
        self.care_plan_mistakes = 0
        self.care_plan_hint_used = False
        self.bedside_misses = 0
    ...
    def handle_event(self, event):
        ...
        if self.active_stage == "overview":
            ...
            if self.bedside_finished and self._continue_button_rect().collidepoint(event.pos):
        ...
        for index, rect in enumerate(self._triage_card_rects()):
            ...
            if case["priority"] == next_priority:
                ...
                if len(self.triage_completed) == len(self.triage_cards):
                    self.skill_scores["triage"] = self._score_performance(1.0, self.triage_mistakes, self.triage_hint_used)
        ...
        for index, rect in enumerate(self._diagnosis_option_rects()):
            ...
            if index == current_round["correct"]:
                ...
                if self.diagnosis_round_index >= len(self.diagnosis_rounds):
                    self.skill_scores["diagnosis"] = self._score_performance(1.0, self.diagnosis_mistakes)
                    self.advance_stage()
            else:
                self.diagnosis_mistakes += 1
                self.diagnosis_feedback = "Not quite. Re-check the symptom pattern before you decide."
            return

    def handle_care_plan_click(self, pos):
        if self._hint_button_rect().collidepoint(pos):
            self.care_plan_hint_used = True
        ...
        if self._action_button_rect().collidepoint(pos):
            if self.care_plan_slots == self.care_plan_order:
                self.skill_scores["accuracy"] = self._score_performance(1.0, self.care_plan_mistakes, self.care_plan_hint_used)
                self.care_plan_feedback = "Correct. Safe care follows a clear sequence."
                self.advance_stage()
            else:
                self.care_plan_mistakes += 1
    ...
    def complete_bedside_rounds(self):
        ...
        success_ratio = self.bedside_hits / len(self.bedside_rounds)
        self.skill_scores["empathy"] = self._score_performance(success_ratio, self.bedside_misses)
    ...
    def _draw_doctor_avatar(self, screen, rect):
        ...
        hair_color = (88, 74, 68)
        coat_color = (248, 248, 247)
        scrub_color = (171, 203, 219)

        head = [
            (center_x - max(26, int(30 * scale)), top_y + max(28, int(32 * scale))),
            (center_x - max(14, int(18 * scale)), top_y + max(6, int(8 * scale))),
            (center_x + max(14, int(18 * scale)), top_y + max(6, int(8 * scale))),
            (center_x + max(26, int(30 * scale)), top_y + max(28, int(32 * scale))),
            (center_x + max(18, int(22 * scale)), top_y + max(64, int(70 * scale))),
            (center_x - max(18, int(22 * scale)), top_y + max(64, int(70 * scale))),
        ]
        hair = [
            (center_x - max(26, int(30 * scale)), top_y + max(26, int(28 * scale))),
            (center_x - max(12, int(16 * scale)), top_y + max(2, int(4 * scale))),
            (center_x + max(22, int(24 * scale)), top_y + max(6, int(8 * scale))),
            (center_x + max(28, int(32 * scale)), top_y + max(30, int(34 * scale))),
            (center_x + max(8, int(10 * scale)), top_y + max(20, int(22 * scale))),
            (center_x - max(16, int(18 * scale)), top_y + max(18, int(20 * scale))),
        ]
        torso_top = top_y + max(80, int(88 * scale))
        torso_bottom = rect.bottom - max(28, int(34 * scale))
        coat = [
            (center_x - max(66, int(74 * scale)), torso_bottom),
            (center_x - max(46, int(52 * scale)), torso_top + max(12, int(14 * scale))),
            (center_x - max(18, int(20 * scale)), torso_top),
            (center_x + max(18, int(20 * scale)), torso_top),
            (center_x + max(46, int(52 * scale)), torso_top + max(12, int(14 * scale))),
            (center_x + max(66, int(74 * scale)), torso_bottom),
        ]
        scrub = [
            (center_x - max(20, int(24 * scale)), torso_top + max(2, int(4 * scale))),
            (center_x + max(20, int(24 * scale)), torso_top + max(2, int(4 * scale))),
            (center_x + max(10, int(12 * scale)), torso_top + max(44, int(48 * scale))),
            (center_x - max(10, int(12 * scale)), torso_top + max(44, int(48 * scale))),
        ]
        neck = [
        ...
        pygame.draw.polygon(screen, scrub_color, scrub)
        ...
        pygame.draw.line(screen, TEXT_COLOR, (center_x - max(10, int(12 * scale)), top_y + max(36, int(40 * scale))), (center_x - max(2, int(4 * scale)), top_y + max(36, int(40 * scale))), 2)
        pygame.draw.line(screen, TEXT_COLOR, (center_x + max(2, int(4 * scale)), top_y + max(36, int(40 * scale))), (center_x + max(10, int(12 * scale)), top_y + max(36, int(40 * scale))), 2)
        pygame.draw.line(screen, TEXT_COLOR, (center_x - max(8, int(10 * scale)), top_y + max(54, int(58 * scale))), (center_x + max(8, int(10 * scale)), top_y + max(54, int(58 * scale))), 2)
        pygame.draw.line(screen, TEXT_COLOR, (center_x - max(18, int(20 * scale)), torso_top + max(18, int(22 * scale))), (center_x, torso_top + max(58, int(64 * scale))), 3)
        pygame.draw.line(screen, TEXT_COLOR, (center_x + max(18, int(20 * scale)), torso_top + max(18, int(22 * scale))), (center_x, torso_top + max(58, int(64 * scale))), 3)
        pygame.draw.circle(screen, TEXT_COLOR, (center_x, torso_top + max(60, int(66 * scale))), max(4, int(5 * scale)))
